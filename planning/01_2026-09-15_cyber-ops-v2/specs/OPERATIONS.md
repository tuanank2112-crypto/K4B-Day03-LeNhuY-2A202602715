...
- **R12-B fresh migration:** target absent → migrate → fault-injection trước COMMIT → không có schema nửa vời; rerun sạch tạo đúng seed/count.
...
Machine output, không sửa tay; mọi file evidence BẮT BUỘC đuôi `.txt` theo HANDOFF_PROTOCOL §14:
...
├── fresh-migrate.txt
├── fresh-manifest.txt
├── existing-pre-manifest.txt
├── backup-result.txt
├── backup-verify.txt
├── rollback-existing.txt
├── rollback-restored-manifest.txt
